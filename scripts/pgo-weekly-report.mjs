...
import { gather, arAsOf, bqReady } from "./pgo-bq.mjs";
...
// ---- report body ----
function buildHtml(f, wow, driveLink, week) {
  const moM =
...
async function main() {
  ...
  const html = buildHtml(f, wow, null, week);
  ...
  try {
    ...
    const emailHtml = buildHtml(f, wow, drive.webViewLink, week);
    await sendEmail(token, MAIL_TO, `PGO Weekly — NOI ${usd(f.noi)}, A/R ${usd(f.ar_total)} (${week})`, emailHtml);
