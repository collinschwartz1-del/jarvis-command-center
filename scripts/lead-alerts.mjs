...
async function fetchHotLeads(sinceISO) {
  // leads a caller marked "Interested" (status -> qualified) since last run
  const q = new URLSearchParams({
    status: "eq.qualified",
    updated_at: `gt.${sinceISO}`,
    order: "updated_at.desc",
    select: "id,score,summary,payload,hub_property(display_address,city)",
  });
  const r = await fetch(`${URL.replace(/\/$/, "")}/rest/v1/hub_lead?${q}`, {
    headers: { apikey: KEY, Authorization: `Bearer ${KEY}` },
  });
  if (!r.ok) return [];
  return (await r.json()).map((l) => ({
    display_address: l.hub_property?.display_address || "(address)",
    owner_name: l.payload?.owner_name || null,
    equity_capture: l.payload?.equity_capture_mid_usd || null,
...
function hotHTML(hot) {
  ...
  const rows = hot.map((l) => `
    <tr><td style="padding:7px 10px;border-bottom:1px solid #2a3340;font-weight:600">${l.display_address}</td>
    <td style="padding:7px 10px;border-bottom:1px solid #2a3340">${l.owner_name ?? "—"}</td>
    <td style="padding:7px 10px;border-bottom:1px solid #2a3340;color:#3fb950;font-weight:700">${usd(l.equity_capture)}</td></tr>`).join("");
  return `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;background:#0a0e14;color:#e6edf3;padding:22px 22px 4px">
    <h2 style="margin:0 0 4px;color:#3fb950">🔥 HOT — sellers marked Interested (${hot.length})</h2>
    <p style="color:#8b96a5;margin:0 0 12px">Follow up fast. Full notes in the Jarvis Deals tab.</p>
    <table style="width:100%;border-collapse:collapse;font-size:13px">
      <tr style="color:#8b96a5;text-transform:uppercase;font-size:11px"><td style="padding:7px 10px">Address</td><td style="padding:7px 10px">Owner</td><td style="padding:7px 10px">Equity</td></tr>${rows}
    </table></div>`;
}
...
async function emailAll(leads, hot) {
  const token = await gmailToken();
  if (!token) { console.log("Gmail creds absent — cannot email."); return; }
  const to = RECIPIENTS.map((r) => r.email).join(", ");
  const subject = hot.length
    ? `🔥 ${hot.length} HOT + ${leads.length} new lead${leads.length === 1 ? "" : "s"} — Omaha Deal Engine`
    : `🏠 ${leads.length} new deal lead${leads.length > 1 ? "s" : ""} — Omaha Deal Engine`;
  const body = hotHTML(hot) + (leads.length ? emailHTML(leads) : "");
  const raw = [
...
async function main() {
  ...
  const [leads, hot] = await Promise.all([fetchNewLeads(since), fetchHotLeads(since)]);
  console.log(`Lead alerts · since ${since} · ${hot.length} HOT · ${leads.length} new acreage/MF${SEND ? "" : " · DRY RUN"}`);
  for (const l of hot) console.log(`  🔥 ${l.display_address} ${usd(l.equity_capture)} — ${l.owner_name ?? ""}`);
  for (const l of leads) console.log(`  • ${l.display_address} [${l.buy_box}] ${l.rank_label ?? ""} ${usd(l.equity_capture)} — ${l.owner_name ?? ""}`);
  if (!leads.length && !hot.length) { console.log("nothing new — no alert."); return; }
  if (!SEND) { console.log("\n(DRY RUN — re-run with --send to email Collin + Tyler. State not advanced.)"); return; }
  await emailAll(leads, hot);
  await smsAll(leads);
  saveRun(nowISO);
}
